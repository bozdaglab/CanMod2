...
# to run this function just put pairwise.corr.target.genes.modules()
pairwise.corr.target.genes.modules<-function(){
  sig<-0
  pairwise.cor.target.gene<-lapply(1:length(dtc.final.module.list), function(i){
...
#run dtc.final.target.list.
# run GO.ES.Score(), KEGG.ES.Score(), CH.ES.Score()
...
dtc.final.target.list<-lapply(1:length(dtc.final.module.list),function(i){
  
  target.list<-dtc.final.module.list[[i]][["targets"]]
  return (target.list)
  
})

GO.ES.Score<-function(){
  ...
  go.enrich.module.all<-lapply(1:length(dtc.final.target.list),function(i){
  ...
  selected.go.enrich.module<-lapply(1:length(dtc.final.target.list),function(i){
    module<-dtc.final.target.list[[i]]
    module = unlist(unname(getBM(attributes = c('entrezgene_id'), filters = 'hgnc_symbol', values=module, mart=ensembl)))
  ...
  module.go.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.go.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyingo.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyingo.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
...
KEGG.ES.Score<- function(){
  ...
  kegg.enrich.module.all<-lapply(1:length(dtc.final.target.list),function(i){
    module<-dtc.final.target.list[[i]]
    module = unlist(unname(getBM(attributes = c('entrezgene_id'), filters = 'hgnc_symbol', values=module, mart=ensembl)))
  ...
  selected.kegg.enrich.module<-lapply(1:length(dtc.final.target.list),function(i){
    module<-dtc.final.target.list[[i]]
    module = unlist(unname(getBM(attributes = c('entrezgene_id'), filters = 'hgnc_symbol', values=module, mart=ensembl)))
  ...
  module.kegg.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.kegg.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyinkegg.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyinkegg.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
...
CH.ES.Score<-function(){
  ...
  ch.enrich.module.all<-lapply(1:length(dtc.final.target.list),function(i){
    module<-dtc.final.target.list[[i]]
    module = unlist(unname(getBM(attributes = c('entrezgene_id'), filters = 'hgnc_symbol', values=module, mart=ensembl)))
  ...
  selected.ch.enrich.module<-lapply(1:length(dtc.final.target.list),function(i){
    module<-dtc.final.target.list[[i]]
    module = unlist(unname(getBM(attributes = c('entrezgene_id'), filters = 'hgnc_symbol', values=module, mart=ensembl)))
  ...
  module.ch.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.ch.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyinch.es.mean.list<-lapply(1:length(dtc.final.target.list), function(x) {
  ...
  module.onlyinch.es.median.list<-lapply(1:length(dtc.final.target.list), function(x) {
